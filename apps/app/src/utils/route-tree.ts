...
export function buildRouteTree(routeTree: AnyRoute, permissions: string[] = []): RouteTreeNode[] {
  ...
  collect(routeTree as unknown as RouteLike)
  ...
  for (const route of routes) {
    ...
    // if (staticData.permission && !permissions.includes(staticData.permission)) continue
    ...
    if (staticData.groupName) {
      ...
      if (existingIndex === undefined) {
        ...
        result.push({
          id: staticData.groupName, // Use groupName as ID for stability
          isGroup: true,
